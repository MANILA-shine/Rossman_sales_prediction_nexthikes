...
def index():
    ...
    # Add the formatted predicted sales and week numbers to the DataFrame
    predictions_df = pd.DataFrame({'Date': date_list, 'Week': range(1, 7), 'PredictedSales': predicted_sales})

    # Load the DataFrame from "sales_predictions.csv"
    # Replace "path_to_csv_file" with the actual path to the CSV file
    predictions_df_from_csv = pd.read_csv("C:/Users/manil/ROSSMAN_SALES_PREDICTION_BY_MANILA/sales_predictions.csv")
